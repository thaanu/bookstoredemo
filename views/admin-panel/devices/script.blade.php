<script>
    ...
    function getDevices() {
        hfPostRequest('/cp/devices/ajax/fetch-user-devices').then(response => {
            if (response.status == 200) {
                ...
                _a('#add-device-btn').addEventListener('click', function(e) {
                    myOffcanvas.querySelector('#pageOffCanvasLabel').innerHTML = 'Add Device';
                    myOffcanvas.querySelector('.offcanvas-body').innerHTML = loadingContent;
                    bsOffcanvas.show();
                    handleForm('/cp/devices/ajax/add-device-form', myOffcanvas.querySelector('.offcanvas-body'), bsOffcanvas);
                });
