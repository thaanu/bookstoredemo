        ...
        INTFO_PATIENT_BIODATA.EVENT_SEQUENCE_NO,   
            INTFO_PATIENT_BIODATA.PATIENT_PRN,     
                ...
                INTFO_PATIENT_BIODATA.EMAIL_CONTACT from (  
                    select to_number(event_sequence_no) as event_no,  prn, account_number , event_name, process_Date_time  FROM TTH_PROD_INTF_OUT.intfo_events   
                    where  event_type = 'ADT' and ( event_name = 'E1') and (processed_flg = 'Y') ) ad left join   
                    TTH_PROD_INTF_OUT.INTFO_PATIENT_BIODATA on TTH_PROD_INTF_OUT.INTFO_PATIENT_BIODATA.event_sequence_no = ad.event_no where PATIENT_ID_NUMBER = '[::SEARCH_QUERY::]' or prn = '[::SEARCH_QUERY::]' order by event_no desc FETCH FIRST 1 ROWS ONLY